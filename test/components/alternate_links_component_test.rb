...
class AlternateLinksComponentTest < ViewComponent::TestCase
  test "renders a x-default link for the current locale" do
    with_request_url "/electricians" do
      render_inline(AlternateLinksComponent.new)
      assert_selector "link[rel=alternate][href='http://test.host/electricians'][hreflang=x-default]", visible: false
    end
  end
  ...
  test "renders links for other locales" do
    with_request_url "/electricians" do
      ...
      (I18n.available_locales - [I18n.locale]).each do |locale|
        assert_selector "link[rel=alternate][href='http://test.host/#{locale}/electricians'][hreflang=#{locale}]", visible: false
      end
    end
  end
